...
class CAppXmlHelper
{
	 ...
	 */
	static public function PopulateAccount($oAccountNode, &$oAccount, $oDomain, $bIsUpdate = false)
	{
...
//			wm_Custom::StaticUseMethod('ChangeFakeEmailToAccountEmail', array(&$sEmail));
		...
		if ($bIsUpdate && $oAccount->IsEnabledExtension(CAccount::ChangePasswordExtension))
		{
			if ($bIsUpdate)
			{
				$sIncomingMailPassword = (string) $oAccountNode->GetChildValueByTagName('mail_inc_pass');
	 ...
	 * @return CContact
	 */
	static public function GetContact($oRequestXml, $oAccount)
	{
	 ...
	 */
	static public function GetMessage($oMessageNode, $oAccount, $oFromAccount, $oFromIdentity = null)
	{
...
//			wm_Custom::StaticUseMethod('ChangeAccountEmailToFake', array(&$mTemp));
		...
		if ($oAttachmentsNode != null)
		{
			...
			$sWebUrl = strtolower(substr($_SERVER['SERVER_PROTOCOL'], 0, strpos($_SERVER['SERVER_PROTOCOL'], '/')))
				.'://'.$_SERVER['HTTP_HOST']
			...
			foreach ($aAttachmentsKeys as $sKey)
			{
				...
				if (!$oMessage->Attachments->AddFromBinaryBody($oTempFiles->LoadFile($oAttachmentNode->GetChildValueByTagName('temp_name')),
	 ...
	 */
	static public function ReplySetFlag($oMessageNode, $oMailProcessor)
	{
	 ...
	 * @return Filter
	 */
	static public function GetConfirmationMessage($oRequestXml, &$oResultXml, $oAccount)
	{
		$oMessage = null;
...
//			wm_Custom::StaticUseMethod('ChangeAccountEmailToFake', array(&$emailAccount));
	 ...
	 * @return Filter
	 */
	static public function GetFilter($oFilterNode)
	{
		$oFilter = null;
	 ...
	 * @return CIdentity
	 */
	static public function GetIdentity($oIdentityNode, $bIsNew = false)
	{
		$oIdentity = null;
