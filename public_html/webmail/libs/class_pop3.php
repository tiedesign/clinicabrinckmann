...
class CPOP3 
{
     ...
     * @return bool
     */
    function connect($server, $port = 110, $timeout = 10 , $sock_timeout = 10)
    {
        if($this->socket)
        {
        ...
        */
		...
		$timeout = CApi::GetConf('socket.connect-timeout', 5);
		$sock_timeout = CApi::GetConf('socket.get-timeout', 5);

		CApi::Plugin()->RunHook('webmail-pop3-update-socket-timeouts',
			array(&$timeout, &$sock_timeout));
		
		$this->_log('POP3 : start connect to '.$server.':'.$port);
    ...
    function _getnextstring($isLog = true)
    {
		...
		if ($isLog)
		{
			$this->_log('POP3 < '.ConvertUtils::ShowCRLF($buffer));
    	}
    ...
    function _putline($string, $isLog = true)
    {
		if ($isLog)
		{
			$this->_log('POP3 > '.ConvertUtils::ShowCRLF($string));
    	}
