...
CMessageViewPane.prototype = {
	...
	_setMessageContWidth: function (iWidth)
	{
		if (this._bUseIframe) {
			...
			this._eMessageText.style.width = (iWidth - this._iColPadding - this._iMsgPadding) + 'px';
		}
		else {
			this._eMessageCont.style.width = (iWidth - this._iColPadding - this._iMsgPadding) + 'px';
		}
	},
	...
	resizeWidth: function (iWidth)
	{
		...
		if (!this._bUseIframe && iClientWidth > 18) {
			iClientWidth = iClientWidth - 18;
	...
	resizeHeight: function (iHeight)
	{
		...
		if (this._bUseIframe) {
			...
			this._eMessageText.style.height = (iHeight - iReplyPaneHeight - this._iMsgPadding) + 'px';
		}
		else {
			this._eMessageCont.style.height = (iHeight - this._iMsgPadding) + 'px';
			this._eMessageText.style.height = (iHeight - iReplyPaneHeight - this._iMsgPadding) + 'px';
		}
	},
	...
	{
		if (this._bUseIframe) {
			this._eMessageText.contentWindow.document.body.innerHTML = html;
		}
		else {
	...
	_updateDirClass: function()
	{
		...
		if (this._bUseIframe) {
			this._eMessageText.contentWindow.document.body.className = sClassName;
		}
		else {
	...
	{
		var eAttachments = CreateChild(this._eMainCont, 'div');
		this._eAttachments = eAttachments;
		eAttachments.style.position = 'absolute';
		eAttachments.style.top = '0px';
		eAttachments.className = 'wm_hide';
		this._bHasAttachments = false;
	},

	_buildResizer: function ()
	{
		var eResizer = CreateChild(this._eMainCont, 'div');
		this._eResizer = eResizer;
		eResizer.style.position = 'absolute';
		eResizer.style.top = '0px';
		eResizer.className = 'wm_hide';
	},
	...
	_buildMessage: function ()
	{
		...
		if (this._bUseIframe) {
			this._eMessageText = CreateChild(this._eMessageCont, 'iframe', [['style', 'border: solid 0px white;']]);
		}
		else {
