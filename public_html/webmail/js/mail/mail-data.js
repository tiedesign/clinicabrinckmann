...
CMessage.prototype = {
					...
					if (!isInAngleBrackets && !isInBrackets && !isInQuotes) {
						...
						if (Trim(str).length > 0) {
							...
							for (var j = 0; j < jCount; j++) {
								if (aRecipients[j].email == sRecipient.email) inList = true;
							}
							if (!inList) {
								aRecipients.push(sRecipient);
							}
						}
	...
	prepareForReply: function (msg, replyAction, fromAddr)
	{
		...
		switch (replyAction) {
			...
			case TOOLBAR_REPLYALL:
				var sToAddr = (msg.replyToAddr.length > 0) ? msg.replyToAddr : msg.fromAddr;
				var aToRecipients = this._getArrayRecipients(sToAddr);
				if (aToRecipients.length > 0) {
					this.toAddr = aToRecipients[0].FullEmail;
				}
				else {
					this.toAddr = '';
				}
	...
	getInXml: function()
	{
		...
		for (var j = 0; j < this.attachments.length; j++) {
			...
			atAttrs += (attachment.inline) ? ' inline="1"' : ' inline="0"';
	...
	{
		this.attachments = [];
		var attachmentsNode = XmlHelper.getFirstChildNodeByName(rootElement, 'attachments');
		if (attachmentsNode == null) return;
		for (var i = 0; i < attachmentsNode.childNodes.length; i++) {
			var attachNode = attachmentsNode.childNodes[i];
			...
			if (sVoice != '#') {
				...
				this.voiceAttachment = { sDownload: sDownload, sVoice: sVoice, iDuration: iDuration,
					sTranscription: '' };
			}
			else {
				...
				this.attachments.push({ id: id, inline: bInline, fileName: sFileName, size: iSize,
...
CFolderList.prototype = {
	...
	getFolderName: function (iId, iFullName)
	{
		var iCount = this.folders.length;
		for (var i = 0; i < iCount; i++) {
			var folder = this.folders[i];
			if (folder.id === iId && folder.fullName === iFullName) {
...
CUpdate.prototype = {
	...
	getFromXml: function(rootElement)
	{
		this.value = XmlHelper.getAttributeByName(rootElement, 'value', this.value);
		switch (this.value) {
			case 'save_message':
				var id = XmlHelper.getIntAttributeByName(rootElement, 'id', -1);
				var uidNode = XmlHelper.getFirstChildNodeByName(rootElement, 'uid');
				var uid = XmlHelper.getFirstChildValue(uidNode, '');
				if (uid == '-1') uid = '';
				this.saveMessage = { id: id, uid: uid };
				break;
			case 'account_properties':
				var accountNode = XmlHelper.getFirstChildNodeByName(rootElement, 'account');
				var accountId = XmlHelper.getIntAttributeByName(accountNode, 'id', null);
				var imapQuota = XmlHelper.getIntAttributeByName(accountNode, 'imap_quota', null);
				var imapQuotaLimit = XmlHelper.getIntAttributeByName(accountNode, 'imap_quota_limit', null);
