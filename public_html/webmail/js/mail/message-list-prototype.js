...
var MessageListPrototype = {
	...
	_fillReplyPane: function (msg) {
		if (this.id == SCREEN_MESSAGE_LIST_CENTRAL_PANE) {
			var msgInSentFolder = this.isSent(msg.idFolder, msg.folderFullName);
			var msgInDraftsFolder = this.isDrafts(msg.idFolder, msg.folderFullName);
			if (WebMail.Settings.allowReplyMessage && !msgInSentFolder && !msgInDraftsFolder) {
		...
		if (null != args) {
			if (args.AcctChanged) {
				...
				if (undefined === args.oKeyMsgList && (this.oKeyMsgList.iFolderId !== -1 || this.oKeyMsgList.sLookFor.length !== 0)
					...
					if (args.oKeyMsgList !== null && args.oKeyMsgList.iFolderId !== -1 || args.oKeyMsgList.sLookFor.length !== 0) {
 ...
 */
	...
	isMessagesOperationEnable: function (type, msgIdArray, toFolder)
	{
		...
		switch (type) {
			case TOOLBAR_DELETE:
				if (this.DeleteLikePop3()) {
					...
					if ((!WebMail.allowTrashTools() || this.isTrash()) && !this.isSpam()) {
						if (!confirm(Lang.ConfirmAreYouSure)) return false;
					}
				...
				if (this.DeleteLikeImap()) {
					...
					if (!confirm(Lang.ConfirmDirectModeAreYouSure)) return false;
			...
			case TOOLBAR_PURGE:
			case TOOLBAR_EMPTY_SPAM:
				if (!confirm(Lang.ConfirmMessagesPermanentlyDeleted)) return false;
				break;
			case TOOLBAR_IS_SPAM:
			case TOOLBAR_NOT_SPAM:
	...
	_getMessagesOperationXmlHeader: function (toFolder, workFolder)
	{
	...
	{
		var workFolder = this._getMessagesOperationWorkFolder(type);

		var xmlHeader = this._getMessagesOperationXmlHeader(toFolder, workFolder);
		var xmlMessages = '';
		var viewMessageForOperation = false;
		...
		WebMail.DataSource.request({action: 'operation_messages', request: OperationTypes[type]},
			xml);
 ...
 */
	...
	FillByFolders: function() {
		...
		for (var key in arFolders) {
			...
			if (typeof(thisFolder) === 'function') continue;
			...
			if (!thisFolder.listHide) {
				...
				var eMoveItem = {};
				if ((notDirectMode || notPop3) && thisFolder.noselect === false) {
					var isInboxFolder = (thisFolder.id == inbox.id && thisFolder.fullName == inbox.fullName);
					eMoveItem = this.addToMoveMenu(thisFolder.id, thisFolder.fullName, thisFolder.strIndent + fName, isInboxFolder);
					this.addToSearchFolders(thisFolder.strIndent + fName, thisFolder.id, thisFolder.fullName,  thisFolder.type);
				}

				var paramIndex = thisFolder.id + thisFolder.fullName;
				var oFldParams = this._foldersParam[paramIndex];
				if (!oFldParams) {
					oFldParams = new CFolderParams(thisFolder.id, thisFolder.fullName,
						thisFolder.sentDraftsType, thisFolder.type, thisFolder.syncType,
						thisFolder.msgCount, thisFolder.newMsgCount, fName, thisFolder.intIndent,
						thisFolder.noselect, eMoveItem);
				}
				...
				oFldParams.setContainer(div, clickHandler);
				if (this.oKeyMsgList.isEqualFolder(thisFolder.id, thisFolder.fullName)) {
					this.ChangeCurrFolder(thisFolder.id, thisFolder.fullName, div, oFldParams.iMsgsCount, thisFolder.syncType, thisFolder.type);
				}
				...
				if (thisFolder.noselect === false) {
					this._dragNDrop.addDropObject(div);
				}
				this._foldersParam[paramIndex] = oFldParams;
			}
		} //for
		this.RepairToolBar();
		this.hideSearchFolders();
