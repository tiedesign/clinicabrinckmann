...
CPopupAutoFilling.prototype =
{
	...
	Replace: function ()
	{
		...
		var pHeight = this._popup.offsetHeight - vertBordersWidth;
		var iMaxHeight = GetHeight() / 2;
		if (pHeight > iMaxHeight) {
			this._popup.style.height = Math.round(iMaxHeight) + 'px';
			this._popup.style.overflowY = 'auto';
		}
		else {
			/*set popup height in absolute value for hiding select under popup in ie6*/
			this._popup.style.height = pHeight + 'px';
			this._popup.style.overflowY = 'hidden';
		}
	},
	...
	fill: function (itemsArr, keywordStr, lastPhrase)
	{
		...
		for (var i = 0; i < iCount; i++) {
            if (null === (new RegExp(itemsArr[i].clearEmail, 'gi')).exec(this._suggestInput.value)) {
                iCountResults++;
...
CPopupContacts.prototype =
{
	...
	Replace: function ()
	{
		if (this._shown)
		{
			...
			var pHeight = this._popup.offsetHeight - bordersHeight - paddingHeight;
			var iMaxHeight = GetHeight() / 2;
			if (pHeight > iMaxHeight) {
				this._popup.style.height = Math.round(iMaxHeight) + 'px';
				this._popup.style.overflowY = 'auto';
			...
			var pWidth = this._popup.offsetWidth * 2 - this._popup.clientWidth;
			this._popup.style.width = pWidth + 'px';
	...
	SetSuggestions: function (suggestionStr)
	{
		...
		var inputValue = Trim(this._suggestInput.value).replace(/,$/, '');
