...
CUserSettingsScreen.prototype = {
			...
			switch (data.type) {
				case TYPE_ACCOUNT_LIST:
					...
					if (this._oAccountsPane.shown) {
						this._oAccountTabsSwitcher.show(this._allowFilters, this._allowAutoresponder, this._oAccountPropertiesPane.getTabLangField(this.bNewMode));
					}
	...
	_showTabs: function (currentTabId, iCurrAccountTab)
	{
		...
		if (showSettingsSwitcher) {
			this._oAccountTabsSwitcher.show(this._allowFilters, this._allowAutoresponder, sAcctPropTabLangField, iCurrAccountTab);
		}
		else {
	...
	_changeAccountId: function(id) {
		...
		if (this.Autoresponders[id]) {
			this._oAutoresponderPane.SetAutoresponder(this.Autoresponders[id]);
		}
