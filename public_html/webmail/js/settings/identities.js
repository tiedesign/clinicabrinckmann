/*
 * Functions:
 *  CreateRemoveIdentityClickFunc(id)
 *  CreateEditIdentityClickFunc(id)
 * Classes:
 *  CIdentitiesPane(oParent)
 */

function CreateRemoveIdentityClickFunc(iId, oIdentitiesPane)
{
	return function () {
...
//		HtmlEditorField.makeActive();
	...
	_fillTable: function ()
	{
		...
		for (var i = 0; i < aIdentities.length; i++) {
			...
			eDeleteLink.onclick = CreateRemoveIdentityClickFunc(oIdentity.iId, oIdentitiesPane);
		}
	...
	_getNewIdentity: function ()
	{
		...
		if (oIdentity.bHtmlSignature) {
			var sHtmlSignature = HtmlEditorField.getHtml();
