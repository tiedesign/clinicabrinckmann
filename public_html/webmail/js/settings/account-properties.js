...
CAccountTabsSwitcher.prototype = {
	...
	show: function(bAllowFilters, bAllowAutoresponder, sTabLangField, iCurrAccountTab) {
		if (iCurrAccountTab === undefined) {
...
CAccountPropertiesPane.prototype = {
	...
	{
		this.shown = false;
		if (this.hasChanges && confirm(Lang.ConfirmSaveAcctProp)) {
			this.saveChanges();
		}
		this.hasChanges = false;
		...
		if (!newAcctProp.bNew && newAcctProp.allowChangePassword && !WebMail.bIsDemo) {
			...
			if (bPassChanged) {
				if (newAcctProp.curPass.length === 0) {
					alert(Lang.AccountOldPasswordsDoNotMatch);
					return false;
				}
