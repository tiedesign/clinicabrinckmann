...
function Init() {
	...
	WebMail = new CWebMail(Title, sLookFor);
...
function CWebMail(title, sLookFor){
...
CWebMail.prototype = {
	...
	getNearMessages: function (id, uid, oWindow)
	{
		this._oWaitMsgListForFirstMsg = null;
		this._oWaitMsgListForLastMsg = null;
		...
		if (oListScreen) {
			...
			do {
				...
				if (oMsgList !== null) {
					oNearMessages = oMsgList.getNearMessages(id, uid);
				}
	...
	requestMessageListNextPage: function (bBackground)
	{
		...
		if (oListScreen) {
			...
			if (iPage > 0) {
				...
				GetHandler(TYPE_MESSAGE_LIST, {idAcct: WebMail.iAcctId, page: iPage, sortField: oKeyMsgList.iSortField,
	...
	_onMessageSending: function ()
	{
		...
		if (oCurrScreen && this.ScreenId === SCREEN_NEW_MESSAGE) {
			oCurrScreen.isSavedOrSent = true;
			SetHistoryHandler(
				{
	...
	_onMessageSaving: function (oSaveMsg)
	{
		...
		if ((oSaveMsg.id === -1 || oSaveMsg.uid === '') && this.ScreenId === SCREEN_NEW_MESSAGE) {
			SetHistoryHandler(
				{
					ScreenId: this.listScreenId,
					idFolder: null
	...
	getFolderName: function (iId, iFullName)
	{
		if (this.folderList != null && this.folderList.idAcct == this.Accounts.currId) {
			return this.folderList.getFolderName(iId, iFullName);
		}
		var folderList = this.Accounts.getCurrentFolderList();
		return  (folderList != null) ? folderList.getFolderName(iId, iFullName) : '';
	},
	...
	LogOut: function (errorCode)
	{
		if (errorCode) {
			...
			Cookies.erase('awm_autologin_data');
			Cookies.erase('awm_autologin_id');
			if (parent) {
...
function CSettingsList()
{
	...
	this.horizResizerPos = 150;
	this.msgResizerPos = 620;
	this.vertResizerPos = 115;
...
CSettingsList.prototype = {
	...
	_readFromCookies: function ()
	{
		...
		for (var i = 0; i < iCount; i++) {
			...
			InboxHeaders[i].Width = Cookies.readInt('wm_column_' + i, InboxHeaders[i].Width);
		}
	}
};
