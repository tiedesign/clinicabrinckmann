...
CVariableTable.prototype = 
{
	...
	{
		var a = CreateChild(parent, 'a', [['href', '#'],['class', 'wm_search_result_clear_button']]);
		a.onclick = function () {
	...
	fill: function (objsArr, screenId, msg, addClearLink, bNextPageMsgList)
	{
		...
		for (var i = 0; i < objsArr.length; i++) {
			...
			for (var j = 0; j < this._columnsCount; j++) {
				...
				td.style.paddingLeft = column._padding + 'px';
				td.style.paddingRight = column._padding + 'px';
	...
	_clickCheckLine: function (id)
	{
		if (id < 0) return;
		this.oSelection.CheckLine(id);
	...
	build: function (parent)
	{
		...
		var headers = CreateChild(div, 'div', [['class', 'wm_inbox_headers']]);
