...
class CInstaller
{
	...
	function CInstaller()
	{
		...
		$this->_aMenu = array('compatibility', 'license', 'license-key', 'db',
			'mobyle-sync', 'admin-panel', 'email-server-test', 'completed'
		);
		
		if (file_exists(WM_INSTALLER_PATH.'../libraries/afterlogic/common/lite.php')) {
			// Lite version steps
			unset( $this->_aMenu[2] ) ; // 'license-key' 
			unset( $this->_aMenu[4] ) ; // 'mobyle-sync' 
			unset( $this->_aSteps['license-key'] ) ;
			unset( $this->_aSteps['mobyle-sync'] ) ;
		}
