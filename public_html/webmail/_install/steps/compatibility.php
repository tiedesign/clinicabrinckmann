...
class CCompatibilityStep extends AInstallerStep
{
	...
	function Validate()
	{
		...
		$this->aCompatibility['php.version.valid'] = (int) (version_compare($this->aCompatibility['php.version'], '5.1.0') > -1);
	...
	function TemplateValues()
	{
		return array(
			'PhpVersion' => ($this->aCompatibility['php.version.valid'])
				...
				: $this->getErrorHtmlValue($this->aCompatibility['php.version'].' detected, 5.1.0 or above required.',
...
or contact your hosting provider and ask to do this.'),
			...
			'MySQL' => ($this->aCompatibility['mysql.valid'])
				? $this->getSuccessHtmlValue('OK')
				: $this->getErrorHtmlValue('Error, PHP MySQL extensions not detected',
'You need to install PHP extension for the appropriate database engine (MySQL).'),
...
Make sure you completely copied the data folder with all its contents
from AfterLogic Webmail installation package into the location you specified in inc_settings_path.php file.'),
