...
echo "managers not inited<br>\n";
		...
		$accountOffset =	 $this->_account->GetDefaultTimeOffset();
...
echo "wmContactContainers LOOP ENDED<br>\n";
	...
	//		$this->_container['Temp'] = null;
		...
		if(strlen($fnContactContainer->GetValue('birthday'))>=10) // 2011-12-31
		{
			...
			$wmContactContainer->SetValue('BirthdayYear',	date('Y',$ts)); // 2011
		}
		else
		{
...
//			$fnContactContainer->SetValue('last_name',			'');
	...
	//		$this->_container['Temp'] = null;
		...
		if(strlen($tmp) == 10)
		{	// 2011-12-31 looks like live date
