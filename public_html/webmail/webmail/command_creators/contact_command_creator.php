...
class MySqlContactCommandCreator extends BaseCommandCreator
{
	...
	function GetFullContactsList($idUser, $includeDeleted = false)
	{
		...
		$deletedCondition = 'auto_create = 0 AND';
		if (!$includeDeleted)
		{
	...
	{
		$sql = 'SELECT id_addr AS IdAddress FROM %sawm_addr_book WHERE deleted = 0 AND auto_create = 0 AND id_user = %d';
	...
	{
		$sql = 'SELECT str_id AS StrId FROM %sawm_addr_book WHERE deleted = 0 AND auto_create = 0 AND id_user = %d';
		return sprintf($sql, $this->_dbPrefix, $idUser);
	}
	...
	{
		$sql = 'SELECT id_addr
				FROM %sawm_addr_book WHERE deleted = 0 AND auto_create = 0 AND id_user = %d AND str_id = %s';
