...
class MySqlFunambolContactCommandCreator extends BaseCommandCreator
{
	...
	function GetFullContactsList($funambolUserLogin)
	{
		$sql = '
			...
			WHERE
				userid = %s
		';
		return sprintf($sql, $this->_escapeString($funambolUserLogin));
	}
    ...
    {
        $sql = 'DELETE FROM fnbl_pim_contact WHERE id = %d';
        $sql = sprintf($sql, $idContact);
        return $sql;
    }
	...
	*/
	...
	{
		$sql = 'DELETE FROM fnbl_pim_contact_item WHERE contact = %d';
		$sql = sprintf($sql, $idContact);
		return $sql;
	}
	...
	*/
	...
	{
		$sql = 'DELETE FROM fnbl_pim_contact_photo WHERE contact = %d';
		$sql = sprintf($sql, $idContact);
		return $sql;
	}
	...
	function GetUserIdsUpdatedContacts($dateUpdate)
	{
		...
		";
		return sprintf($sql, $this->_escapeString($dateUpdate));
	}
