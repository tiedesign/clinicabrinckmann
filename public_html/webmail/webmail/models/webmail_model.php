...
class WebMailModel extends BaseModel
{
	...
	protected function _UpdateAccount($account)
	{
		...
		if ($dbStorage && $dbStorage->Connect())
		{
			$oldAcct =& $dbStorage->SelectAccountData($account->IdAccount);
			if ($oldAcct)
			{
		...
		else
		{
			...
			{
				$_fs = new FileSystem(INI_DIR.'/mail', strtolower($oldAcct->Email), $account->IdAccount);
		...
		if ($loginArray != null)
		{
			if ($loginArray[2] == '1')
			{
				if ($password === null)
				{
					...
					$_SESSION[APP_SESSION_ACCOUNT_ID] = $loginArray[0];
					$_SESSION[APP_SESSION_USER_ID] = $loginArray[3];
					return true;
				}	
				...
				else
				{
					...
					if ($newprocessor->MailStorage->Connect(true))
					{
						if ($account->Update())
						{
							...
							$_SESSION[APP_SESSION_ACCOUNT_ID] = $account->IdAccount;
							$_SESSION[APP_SESSION_USER_ID] = $account->IdUser;
	...
	protected function _GetApplicationBaseUrl($startPage = null, $toEmail = null, $isSeparated = false)
	{
		...
		if (!isset($_SESSION[APP_SESSION_ACCOUNT_ID]) || !isset($_SESSION[APP_SESSION_USER_ID]))
		{
	...
	protected function _SendMessage($account, $to, $subject, $bodyText, $isBodyHtml = false, $attachmentsFileName = array())
	{
		...
		return CSmtp::SendMail($account, $message, $account->GetFriendlyEmail(), implode(', ', $to));
	}
