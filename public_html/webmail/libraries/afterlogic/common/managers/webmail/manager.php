...
class CApiWebmailManager extends AApiManagerWithStorage
{
	...
	/**
	 * @param CSyncConfig $oSyncConfig
	 * @return bool
	 */
	public function UpdateSyncConfig(CSyncConfig $oSyncConfig)
	{
		$bResult = false;
		try
		{
			if ($oSyncConfig->Validate())
			{
				$oSettings =& CApi::GetSettings();
				$aSettingsMap = $oSyncConfig->GetSettingsMap();

				foreach ($aSettingsMap as $sProperty => $sSettingsName)
				{
					$oSettings->SetConf($sSettingsName, $oSyncConfig->{$sProperty});
				}
	...
	protected function loginToAccountFull($sEmail, $sPassword, $sChangeLang, $aExtValues = null)
	{
		...
		if (is_a($mResult, 'CAccount'))
		{
			CApi::Plugin()->RunHook('statistics.login', array(&$oAccount, null));
		}
	...
	public function GetJsFilesListByType($sType)
	{
		...
		{
			if (CApi::GetConf('js.use-js-gzip', false) && api_Utils::IsGzipSupport())
			{
