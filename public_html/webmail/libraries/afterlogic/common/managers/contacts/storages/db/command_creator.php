...
class CApiContactsCommandCreator extends api_CommandCreator
{
	...
	protected function getContactByWhere($sWhere)
	{
		...
		$sSql = 'SELECT %s FROM %sawm_addr_book WHERE deleted = 0 AND %s';
...
class CApiContactsCommandCreatorMySQL extends CApiContactsCommandCreator
{
	 ...
	 * @return string
	 */
	public function GetSuggestGlobalItems($oAccount, $sSearch, $iRequestLimit)
	{
		$sGABVisibility = '1 = 0';
		$sSearchAdd = $sDomainAdd = '';
		if (EContactsGABVisibility::Off !== $oAccount->GlobalAddressBook)
		{
			...
			{
				if (0 < $oAccount->IdDomain)
				{
					$sDomainAdd = sprintf(' AND id_domain = %d', $oAccount->IdDomain);
				}
				else
				{
					$sDomainAdd = ' AND 1 = 0';
			...
			if (!empty($sSearch))
			{
				...
				$sSearchAdd = sprintf(' AND (email LIKE %s OR friendly_nm LIKE %s)', $sSearch, $sSearch);
			}
		}

		$sSql = 'SELECT id_acct, friendly_nm, email FROM %sawm_accounts
WHERE %s%s%s
LIMIT %d, %d';

		return sprintf($sSql, $this->Prefix(), $sGABVisibility, $sDomainAdd, $sSearchAdd, 0, $iRequestLimit);
	}
