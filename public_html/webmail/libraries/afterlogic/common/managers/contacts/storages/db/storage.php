...
class CApiContactsDbStorage extends CApiContactsStorage
{
	...
	{
		$mContactItems = false;
		if ($this->oConnection->Execute($this->oCommandCreator->GetSuggestGlobalItems(
			$oAccount, $sSearch, $iRequestLimit)))
		{
			$mContactItems = array();
			...
			{
				$oContactItem = new CContactListItem();
				$oContactItem->InitByDbRowWithType('suggest-global', $oRow);
				$mContactItems[] = $oContactItem;
				unset($oContactItem);
			}
		}
