	...
	public function GetByType($sManagerType)
	{
		...
		if (CApi::IsValid())
		{
			...
			else
			{
				...
				if (CApi::Inc('common.managers.'.$sManagerType.'.manager'))
				{
...
abstract class AApiManager
{
	 ...
	 */
	protected function setLastException(CApiBaseException $oException, $bLog = true)
	{
...
abstract class AApiManagerWithStorage extends AApiManager
{
	...
	public function __construct($sManagerName, CApiGlobalManager &$oManager)
	{
		...
		if (CApi::Inc('common.managers.'.$this->GetManagerName().'.storages.'.$this->GetStorageName().'.storage'))
		{
...
abstract class AApiManagerStorage
{
	...
	protected function throwDbExceptionIfExist()
	{
		...
		if ($this->oConnection)
		{
			...
			if (is_a($oException, 'CApiDbException'))
			{
