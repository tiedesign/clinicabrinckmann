...
abstract class BaseManager
{
	...
	{
		$dbPrefix = '';
		$escapeType = QUOTE_ESCAPE;
		$dbType = $settings = $dbDriver = null;
		try
		{
			$settings =& CApi::GetSettings();
			$dbDriver =& CApi::GetManager()->GetConnection();

			$dbType = $settings->GetConf('Common/DBType');
		...
		if (isset($this->_defaultCommandeCreatorNames[$dbType]))
		{
			require_once($this->_defaultCommandCreatorPath);
			$comandCreator = new $this->_defaultCommandeCreatorNames[$dbType]($dbPrefix, $escapeType);
	...
	{
		$this->_lastError = $e;
		CApi::Log('BaseManager: '.$e->getTraceAsString());
	}
}
