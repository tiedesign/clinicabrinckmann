	...
	*/
	...
	public function IsReady()
	{
		if (!$this->_driver)
		{
			...
			$this->_lastError = new BaseDBModelException($errorMsg, $errorCode);
			return false;
		}
	...
	protected function _query($sql)
	{
		...
		$result = $this->_driver->GetResultAsAssocArrays();
		return $result;
	}
	...
	protected function _isDriverAllow()
	{
		if (is_null($this->_driver))
		{
			$errorMsg = self::ERR_MSG_DRIVER_NOT_SET;
			$errorCode = self::ERR_NO_DRIVER_NOT_SET;
			throw new BaseDBModelException($errorMsg, $errorCode);
		}
		return true;
	}
