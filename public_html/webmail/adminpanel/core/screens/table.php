...
class ap_Table_Screen extends ap_Screen
{
	...
	public function PreModuleInit()
	{
		...
		if (isset($_GET['search']) && Cpost::Has('searchdesc'))
		{
			...
			{
				unset($_SESSION[$sScreenName.self::SESS_SEARCH]);
			}
			else
			{
				$_SESSION[$sScreenName.self::SESS_SEARCH] = $sSearchDesc;
			}
		}
		...
		{
			$_SESSION[$sScreenName.self::SESS_PAGE] = (int) $_GET['page'];
		}

		if (isset($_SESSION[$sScreenName.self::SESS_PAGE]))
		{
			$this->iPage = (int) $_SESSION[$sScreenName.self::SESS_PAGE];
		}

		if (isset($_SESSION[$sScreenName.self::SESS_SEARCH]))
		{
			$this->sSearchDesc = $_SESSION[$sScreenName.self::SESS_SEARCH];
		}
		...
		{
			$_SESSION[$sScreenName.self::SESS_ORDERBY] = $_GET['scolumn'];
		}
		if (isset($_GET['sorder']) && is_numeric($_GET['sorder']))
		{
			$_SESSION[$sScreenName.self::SESS_ORDERTYPE] = (int) $_GET['sorder'];
		}

		if (isset($_SESSION[$sScreenName.self::SESS_ORDERBY]))
		{
			$this->sOrderField = $_SESSION[$sScreenName.self::SESS_ORDERBY];
		}
		if (isset($_SESSION[$sScreenName.self::SESS_ORDERTYPE]))
		{
			$this->bOrderType = (bool) $_SESSION[$sScreenName.self::SESS_ORDERTYPE];
		}
	}
...
');
	...
	public function WriteTopMenu()
	{
		...
		if (0 < count($this->aTopMenu))
		{
			...
			{
				if (is_a($oMenuItem, 'ap_Table_Screen_TopMenuItem'))
				{
...
class ap_Table_Screen_ListFilter
{
	...
	public function __construct($sTab, $sName, ap_Table_Screen &$oScreen)
	{
		...
		{
			$_SESSION[$sScreenName.self::SESS_FILTER] = $_GET['filter'];
		}
		$this->sSelectedItem = isset($_SESSION[$sScreenName.self::SESS_FILTER])
			? $_SESSION[$sScreenName.self::SESS_FILTER] : '';
	}
	...
	public function GetSelectedItemKey()
	{
		...
		$sScreenName = $this->oTableScreen->GetScreenName();
		$_SESSION[$sScreenName.self::SESS_FILTER] = (0 < count($aTemp)) ? $aTemp[0] : '';
		$this->sSelectedItem = $_SESSION[$sScreenName.self::SESS_FILTER];
