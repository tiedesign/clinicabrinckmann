...
<div class="wm_tabslist_item_small"><a href="'.AP_INDEX_FILE.'?help" target="_blank">Help</a></div></div>';
	...
	protected function initAdminPanel()
	{
		...
		{
			$_SESSION[AP_SESS_TAB] = isset($_SESSION[AP_SESS_TAB]) ? $_SESSION[AP_SESS_TAB] : AP_TAB_DEFAULT;
		}

		$this->sTab = $_SESSION[AP_SESS_TAB];
		...
		try
		{
			...
			if ($bResetToDefault)
			{
				$_SESSION[AP_SESS_TAB] = $this->IsSubAdminAuthType() ? AP_TAB_SUB_DEFAULT : AP_TAB_DEFAULT;
				$this->sTab = $_SESSION[AP_SESS_TAB];
			}
			...
			else
			{
				...
				if (isset($_SESSION[AP_SESS_ERROR]))
				{
					$this->JsAddInitText('OnlineMsgError("'.
						ap_Utils::ReBuildStringToJavaScript(
							nl2br($_SESSION[AP_SESS_ERROR]), '"').'");');
					
					unset($_SESSION[AP_SESS_ERROR]);
				}
				else if (isset($_SESSION[AP_SESS_MESSAGE]))
				{
					$this->JsAddInitText('OnlineMsgInfo("'.
						ap_Utils::ReBuildStringToJavaScript(
							nl2br($_SESSION[AP_SESS_MESSAGE]), '"').'");');
					
					unset($_SESSION[AP_SESS_MESSAGE]);
				}
			}
		}
	 ...
	 * @return bool
	 */
	static protected function initInclude()
	{
		static $bIsInclude = false;

		$bSessionIsStart = true;
		if (!$bIsInclude)
		{
			$sRp = self::RootPath();
			include_once $sRp.'/../libraries/afterlogic/api.php';
		
			include_once $sRp.'/core/session.php';
			...
			include_once $sRp.'/core/custom.php';
		...
		return $bSessionIsStart;
	}

	/**
	 * @return void
	 */
	static protected function initDataPath()
	{
	...
	{
		$_SESSION[AP_SESS_MESSAGE] = $sDesc;
	}
	 ...
	 * @return string | false
	 */
	static public function AdminDataFolder()
	{
	 ...
	 * @return string
	 */
	static public function ClearAdminVersion()
	{
	 ...
	 * @return string
	 */
	static public function GetWebMailPath()
	{
